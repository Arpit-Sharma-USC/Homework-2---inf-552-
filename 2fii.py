...
for l in range(1,2):
    ...
    for inner in range(1,l+1):
        ...
        heading_conf = 'Confusion matrix for Naive Bayes with Multinomial Prior L=' + str(l)
        ax.set_title(heading_conf)
        plt.show()
        error=mean_squared_error(y_test,preds)

    print('Naive bayes with Multinomial classifier for L=',l,':',np.mean(mean_chunk_arr))
